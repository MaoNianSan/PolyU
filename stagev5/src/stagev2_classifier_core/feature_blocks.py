...
LATE_ACTIVATION_POLICY = "raw_F8_mean"
...
def infer_stage_columns(train_df: pd.DataFrame, test_df: pd.DataFrame) -> dict:
    ...
    return {
        ...
        "late_columns": late,
        "late_raw_f8_columns": late,
        "late_auxiliary_columns": [],
        "late_model_feature_count": len(late),
        "late_diagnostic_column_count": 0,
        "late_auxiliary_used_in_model": False,
        "late_activation_policy": LATE_ACTIVATION_POLICY,
        "n_early": len(early),
        "n_middle": len(middle),
        "n_late": len(late),
    }
...
def _raw_f8_mean_signal(train_block: pd.DataFrame, test_block: pd.DataFrame, name: str):
    """Transparent late activation: row-wise mean of the strict raw F8 model columns."""
    tr = _safe_numeric(train_block)
    te = _safe_numeric(test_block)
    med = tr.median(axis=0).replace([np.inf, -np.inf], np.nan).fillna(0.0)
    strn = tr.fillna(med).mean(axis=1).to_numpy(dtype=float)
    stst = te.fillna(med).mean(axis=1).to_numpy(dtype=float)
    return pd.Series(strn, index=train_block.index, name=name), pd.Series(stst, index=test_block.index, name=name)
...
def build_feature_blocks(train_df: pd.DataFrame, test_df: pd.DataFrame, schema: dict) -> dict:
    ...
    tr_late_sig, te_late_sig = _raw_f8_mean_signal(Xtr_l, Xte_l, "late_activation")
    ...
    out = {
        ...
        "n_late": len(l),
        "late_columns": l,
        "late_activation_policy": schema.get("late_activation_policy", LATE_ACTIVATION_POLICY),
    }
    return out
