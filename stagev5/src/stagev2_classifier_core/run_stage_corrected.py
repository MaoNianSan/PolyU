...
def main() -> None:
    ...
    schema["late_auxiliary_columns"] = source_info.get("ad_late_auxiliary_columns", [])
    schema["late_diagnostic_column_count"] = len(schema["late_auxiliary_columns"])
    schema["late_auxiliary_columns_consistent"] = (
        source_info.get("ad_late_auxiliary_columns", [])
        == source_info.get("control_late_auxiliary_columns", [])
        == source_info.get("test_late_auxiliary_columns", [])
    )
    ...
    run_config = {
        ...
        "late_raw_f8_columns": schema["late_raw_f8_columns"],
        "late_auxiliary_columns": schema["late_auxiliary_columns"],
        "late_auxiliary_used_in_model": schema["late_auxiliary_used_in_model"],
        "late_activation_policy": schema["late_activation_policy"],
