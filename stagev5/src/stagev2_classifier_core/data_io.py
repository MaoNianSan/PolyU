...
STAGEV5_ROOT = Path(__file__).resolve().parents[2]
if str(STAGEV5_ROOT) not in sys.path:
    sys.path.insert(0, str(STAGEV5_ROOT))

from src.feature_adapter import resolve_late_auxiliary_columns, resolve_late_raw_f8_columns
...
def _aggregate_stage_df(df: pd.DataFrame, split: str, stage: str) -> Tuple[pd.DataFrame, list[str]]:
    ...
    if stage == "late":
        feature_cols = resolve_late_raw_f8_columns(df)
    elif stage == "middle":
        feature_cols = [c for c in df.columns if c.startswith("embedding_dim_") and pd.api.types.is_numeric_dtype(df[c])]
    else:
...
def _merge_split(split: str) -> tuple[pd.DataFrame, dict]:
    ...
    sources[f"{split}_late_raw_f8_columns"] = late_cols
    sources[f"{split}_late_auxiliary_columns"] = resolve_late_auxiliary_columns(late_raw, resolve_late_raw_f8_columns(late_raw))
    sources[f"{split}_late_auxiliary_used_in_model"] = False
    return df, sources
