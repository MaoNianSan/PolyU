...
def generate_early_features(train: pd.DataFrame, test: pd.DataFrame, output_root: Path = config.FEATURE_DIR / "early") -> dict[str, dict[str, pd.DataFrame]]:
    ...
    for variant, parts in results.items():
        d = output_root / variant
        d.mkdir(parents=True, exist_ok=True)
        for split, df in parts.items():
            df.to_csv(d / f"{split}_{variant}.csv", index=False, encoding="utf-8")
    return results
...
def early_feature_columns(df: pd.DataFrame) -> list[str]:
    ...
    meta = set(META_COLS)
    return [c for c in df.columns if c not in meta and c not in audit_exclude and c.startswith("early_") and pd.api.types.is_numeric_dtype(df[c])]
