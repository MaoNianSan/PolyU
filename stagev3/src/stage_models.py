...
class StageScoreCalibratedLR(BaseEstimator, ClassifierMixin):
    """v2 OOF stage-score LR using [s_E, s_M, s_L] interactions."""

    def __init__(self, n_early: int, n_middle: int, n_late: int, mode: str = "stage_score", base_C: float = 1.0, meta_C: float = 1.0, class_weight=None, inner_splits: int = 3, random_state: int = 2026):
    ...
    def stage_scores(self, X):
        ...
        X_e, X_m, X_l = self._split(X)
        return (
            self.early_model_.predict_proba(X_e)[:, 1],
            self.middle_model_.predict_proba(X_m)[:, 1],
            self.late_model_.predict_proba(X_l)[:, 1],
        )
...
class MLPSVCLateCalibratedLR(BaseEstimator, ClassifierMixin):
    """v2 meta LR over [p_MLP(early+middle), p_SVC(early+middle), s_L, interactions]."""

    def __init__(self, n_early: int, n_middle: int, n_late: int, meta_C: float = 1.0, late_C: float = 1.0, class_weight=None, inner_splits: int = 3, random_state: int = 2026):
        self.n_early = n_early
        self.n_middle = n_middle
        self.n_late = n_late
        self.meta_C = meta_C
    ...
    def stage_scores(self, X):
        ...
        X_em, X_l = self._split(X)
        return (
            self.mlp_model_.predict_proba(X_em)[:, 1],
            self.svc_model_.predict_proba(X_em)[:, 1],
            self.late_model_.predict_proba(X_l)[:, 1],
        )
